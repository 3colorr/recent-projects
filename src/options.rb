...
class Options
    ...
    private def show_help
        ...
        puts "  list,ls    Show recently opened projects along with 'history id'."
        puts "  apps       Show registered apps and path."
        puts "  add        Register an app along with installed path."
        puts "              (e.g.) rp add <app-name> </path/to/app>"
        puts "  remove,rm  Remove registered app."
        puts "              (e.g.) rp remove <app-name> (or rp rm <app-name>)"
        puts "  config     Show configure list or set value to configure."
        puts "              (e.g.) rp config [config-name] [value]"
        puts "\n--- Quick Start ---"
